        ...
        <template slot-scope="scope">
          <div class="text-overflow">
            <span v-for="(item,index) in scope.row.ar" :title="scope.row.ar | singerNameFormat">
              <router-link to="/" class="singer-link">{{ item.SingerName }}</router-link>
...
export default {
  ...
  methods:{
    getAlbumSongListTable(data){
      this.songList = data
    }
  },
  mounted() {
    this.$bus.$on('getAlbumSongListTable',this.getAlbumSongListTable)
  },
  beforeDestroy() {
    this.$bus.$off('getAlbumSongListTable')
  }
}
</script>
